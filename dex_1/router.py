...
def route(path, method, data, query, headers):
    # حذف پیشوند احتمالی (web_server.py آن را جدا می‌کند، اما محکم‌کاری)
    if path.startswith("/dex_1"):
        path = path[len("/dex_1"):] or "/"

    # استخراج شناسه (id) از انتهای مسیر
    ...
    match (path, method):
        ...
        case ("/swap", "GET"):
            ...
            if not user:
                return response.redirect("/dex_1/login")
            ...
            ctx = build_context(pool_options=pool_options,
        ...
        case ("/register", "GET"):
            ctx = build_context(error="", title="Register",
                                extra_head=f'<link rel="stylesheet" href="{settings.BASE_URL}/static/css/register.css">')
            html = response.render_template("register.html", ctx)
            return response._200(html) if html else response._500()

        case ("/login", "GET"):
            if auth.get_current_user(headers):
                return response.redirect("/dex_1/catalog")
        ...
        case ("/logout", "GET"):
            ...
            return response.redirect("/dex_1", headers_cookie)
        ...
        case ("/admin/pools", "GET") if id:
            return response.redirect("/dex_1/admin/pools")
        ...
        case ("/swap", "POST"):
            ...
            if not row:
                ...
                ctx = build_context(pool_options=pool_options, from_token_options=from_token_options,
                                    to_token_options=to_token_options, error="Invalid pool.", success="", title="Swap",
                                    extra_head='<link rel="stylesheet" href="/static/css/register.css">')
                html = response.render_template("swap.html", ctx)
                return response._200(html) if html else response._500()
        ...
        case ("/my-pools", "POST"):
            ...
            if not pool_id:
                return response._200("<p>Pool ID required.</p><a href='/catalog'>Back</a>")
            success, msg = controllers.user_pool_add.handle(
                user['id'], int(pool_id))
            if success:
                return response.redirect("/dex_1/my-pools")
            return response._200(f"<p>{msg}</p><a href='/catalog'>Back</a>")
        ...
        case ("/login", "POST"):
            ...
            if user:
                ...
                return response.redirect("/dex_1/catalog", headers)
        ...
        case ("/edit_user", "POST") if id:
            ...
            if success:
                return response.redirect("/dex_1/admin/users")
            conn = sqlite3.connect(settings.DB_PATH)
            conn.row_factory = sqlite3.Row
            cur = conn.cursor()
            cur.execute("SELECT * FROM users WHERE id=?", (id,))
        ...
        case _ if path.startswith("/static/"):
            return response.serve_static_file(path)
